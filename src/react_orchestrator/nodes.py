...
def assistant(state: AgentState, config: RunnableConfig):
    ...
    ai_message = AIMessage(
        **reduce(operator.add, ai_message_list).model_dump(exclude={"type"})
    )
...
def check_tool_permission(state: AgentState):
    ...
    tool_chosen = last_msg.tool_calls[
        0
    ]  # Hard-coded assumption is no parallel tool calls possible
    if state["ask_tool_permission"] == "y":
        tool_decision = interrupt(
        ...
        if tool_decision.strip().lower()[0] != "y":
            ...
            tool_reject_message = ToolMessage(
                content=f"The user has denied the invocation of the tool {tool_chosen['name']}",
...
def after_check_tool_permission(state: AgentState):
    ...
    elif isinstance(msg, AIMessage) and msg.tool_calls[0]["name"] in {
        t.name for t in VIRTUAL_TOOLS_FAN_OUT
    }:
...
def research_agent_node(state: AgentState):
    ...
    with (
        _search_log_lock
    ):  # Prevent race conditions if multiple large responses are in fan-out pattern
    ...
    for result in enumerate(resp["results"]):
        content += (
            f"Result number {result[0]}, relevance score: {result[1]['score']}\n"
            f"{result[1]['content']}\n\n"
        )
...
def compile_research(state: AgentState):
    ...
    try:
        assert isinstance(ai_msg_tool_emmision, AIMessage), (
            f"compile_research node fail, last message:\n{ai_msg_tool_emmision}\n>Not an ai message tool call emission"
        )
    except Exception as e:
        raise e
